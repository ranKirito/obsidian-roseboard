...
- Since 1.3, tasks may have an optional `assignee` (up to 100 characters). Day and Calendar use `dueDate`; there is no second scheduling database or automatic recurrence. Documents read referenced Markdown through the host and never copy note bodies into board records.
- Since 1.4, Read expands a card only in the current view; temporary dimensions must not be saved. Inline Markdown editing uses one `NoteSession` per resolved note path, separate from board history, with explicit Save/Cancel, exact-baseline `Vault.process` writes and local note recovery drafts. Never overwrite a concurrent note change or save a truncated preview. Source editors and notes containing boards are guarded.
