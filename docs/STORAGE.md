...
Day and Calendar project existing task `dueDate` values into the board timezone. Quick capture creates an ordinary task with that date. Since 1.5, it also adds one task placement at the first free card-sized grid slot, anchored at the board's top-left card, scanning rows left to right. A slot is free when it keeps 40 px from every existing card and frame. No existing record moves, and placing twice keeps the single placement. Kanban's column **+** and **New → Task** in Day/Calendar use the same rule. Rescheduling changes only `dueDate`; completion changes only status. Completed tasks remain visible on their date and never appear in Overdue or Unscheduled. Filters apply to all task views, including calendar counts and daily progress. There is no recurrence generation, separate calendar store, or cross-board task aggregation.
...
Document cards read linked Markdown through `Vault.cachedRead`. (The separate Documents library view was removed in 1.5; reading happens on cards.) Bodies are never embedded into board JSON. Mounted readers watch vault file events and release their listeners when closed. Collapsed card previews render at most 2,400 characters; expanded readers render at most 100,000 and show a truncation notice. YAML properties are omitted from the preview. Markdown tables, disabled checklist controls and wiki links render without executing raw HTML, code blocks, embeds or other plugins; images are omitted. Since 1.4, card editing can save the linked Markdown directly. The reader does not recursively mount Roseboard blocks, and a missing file is shown as missing rather than replaced.

## Routines (1.5)

An optional top-level `routines` record holds repeating habits shown in Day: `routines[routineId] = {title, days, done, order?}` plus the optional stamps. `title` is 1–200 characters. `days` lists ISO weekdays (1 = Monday … 7 = Sunday), unique, one to seven entries, default all seven. `done` lists the date-only strings (board timezone) on which the routine was ticked, unique. The plugin keeps the newest 400 dates when ticking; validation accepts up to 1,000. `order` is an optional display position; ties sort by title.

Routines are deliberately not tasks. They have no status, due date, placement or dependencies, appear in no task filter or task view, and never generate task records. A board without routines omits the field; removing the last routine removes it. Readers before 1.5 keep `routines` as an unknown extension. Keys sort like the other records.

Merging treats routines like every other record: record-level three-way merge with field stamps. `days` and `done` merge as sets, so ticks made on two devices combine, and an untick on one device removes that date for both. Undo and the activity feed apply.
